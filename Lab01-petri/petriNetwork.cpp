...
bool petri::Network::operator()(){
  ...
  for(auto& t : this->transitions)
    if(t){
      activables.push_back(&t);
      active = true;
    }
  if(active){
    std::uniform_int_distribution<int> random_index(0, activables.size()-1);
    auto t = *(activables.at(random_index(gen)));
    t();
...
std::ostream& petri::operator<<(std::ostream& os, const petri::Network& n){
  // A shared pointer type as pool_ref can be used as map key, since
